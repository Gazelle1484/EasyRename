実機検証結果と、codexの回答から、STEP4-15のプロンプトを生成してください。
...
- 前回使ったディレクトリとcsvのパスを記憶してほしい。アプリがキルされてもアンインストールされない限りは保持されるよう、端末内部に記憶してほしい。
- ホーム画面で一番上に「前回のセット：＜ディレクトリ名＞/<csv名>」を表示しておき、押下すると選択されるようにしてほしい。また、現状の「ディレクトリを選択」・「csvを選択」はそのままにしておくこと。
- 「マッチング画面に進む」は常時表示しておく。デフォルトはグレーアウトしてタップを無効化しておく。ディレクトリとcsvの両方を選択すると現在の色に変化し、タップを有効化する。
- 「マッチング画面に進む」を押下するたびにディレクトリとcsvを読み込んでマッチング画面の表示を最新化すること。
- ディレクトリの読み込みは、ファイルの中身を読み込まず、中のファイル名のみの取得かを確認すること。
- リネームモードの選択式ボタンは、選択する項目の中で最も長い項目に合わせて横幅を小さくする。
- マッチング画面では、小文字のファイル名が大文字になっている。小文字は小文字、大文字は大文字で表示すること。
- マッチング画面で選択中のカードの色をハイライトすること。「[選択中]」の表記は不要のため、仕組みを消去すること。
...
- リネーム前ファイルとリネーム候補が両方選択されると、画面下のリネーム履歴を表示している部分に「選択中：＜リネーム前ファイル名＞->＜リネーム後ファイル名＞」を表示すること。
