...
- マッチング画面を左右に二分割し、左側にリネーム前のファイル、右側にリネーム候補を出したい。それそれの列は独立してスクロールできるようにしたい。
- マッチング画面からホーム画面に遷移するための「戻る」ボタンがほしい。これもandroidのreferenceに従い色をわかりやすく変えてほしい。
